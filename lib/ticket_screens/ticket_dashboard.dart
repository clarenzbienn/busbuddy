...
class _TicketDashboardState extends State<TicketDashboard> {
  ...
  Widget _buildReservationCard(List<Map<String, dynamic>> reservations) {
    // Use the first reservation for terminal, destination, etc.
    ...
    // Combine seats into a string
    String seatNumbers = reservations.map((res) => res['seat']).join(', ');

    // Format the fare as a double with 2 decimal points
  ...
  Widget build(BuildContext context) {
    List<dynamic> filteredReservations = _reservations.where((reservation) {
      String destination = reservation['destination']?.toLowerCase() ?? '';
      return destination.contains(_searchQuery.toLowerCase());
    }).toList();
